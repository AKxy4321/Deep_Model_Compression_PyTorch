...
def my_get_l1_norms_filters_per_epoch(weight_list_per_epoch):
    ...
    num_filters = [
        np.array(weight_list_per_epoch[i]).shape[-1] for i in range(num_layers)
    ]
...
class Get_Weights(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for index, each_weight in enumerate(
            my_get_weights_in_conv_layers(self.model, self.first_time)
        ):
...
def optimize(model, weight_list_per_epoch, epochs, percentage, first_time):
    ...
    print(
        "INITIAL REGULARIZER VALUE ",
        my_get_regularizer_value(model, weight_list_per_epoch, percentage, first_time),
    )
